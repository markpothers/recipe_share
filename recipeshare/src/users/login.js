...
export default connect(
	...
	class LoginScreen extends React.Component {
		...
		componentDidUpdate = () => {
			if (
				!this.state.thanksForRegisteringPopUpShowing &&
				!this.state.thanksForRegisteringPopUpCleared &&
							...
							<View style={centralStyles.formSection}>
								...
								{this.state.loginError && (
									<View style={centralStyles.formErrorView}>
										...
										{this.state.error === "password_expired" && (
											<Text
												testID={"passwordExpiredErrorMessage"}
												maxFontSizeMultiplier={2}
												style={centralStyles.formErrorText}
											>
												Automatically generated password has expired. Please reset your
												password.
											</Text>
										)}
										{this.state.error === "activation" && (
											<Text
												testID={"activationErrorMessage"}
												maxFontSizeMultiplier={2}
												style={centralStyles.formErrorText}
											>
												Account not yet activated. Please click the link in your confirmation
												e-mail. (Don&apos;t forget to check spam!)
											</Text>
										)}
										{this.state.error === "deactivated" && (
											<Text
												testID={"deactivatedErrorMessage"}
												maxFontSizeMultiplier={2}
												style={centralStyles.formErrorText}
											>
												This account was deactivated. Reset your password to reactivate your
												account.
											</Text>
										)}
										{this.state.error === "forgotPassword" && this.props.e_mail.length > 0 && (
											<Text
										...
										{this.state.error === "forgotPassword" && this.props.e_mail.length == 0 && (
											<Text
												...
												style={centralStyles.formErrorText}
											>
												Please enter your e-mail and hit the &apos;Forgot Password&apos; button
												again.
											</Text>
										)}
										{this.state.error === "reactivate" && (
											<Text
