...
export default connect(mapStateToProps, mapDispatchToProps)(
  class ChefDetails extends React.Component {
    static navigationOptions = ({ navigation }) => {
      // console.log(navigation.getParam('followable'))
      return {
        title: 'Chef details',
        ...
        headerRight: (
          <React.Fragment>
          {navigation.getParam('followable') == false ? <Button rounded style={styles.newButton} onPress={navigation.getParam('unfollowChef')}><Icon name='account-multiple-plus' size={28} style={styles.newIcon}/></Button> : <Button rounded style={styles.newButton} onPress={navigation.getParam('followChef')}><Icon name='account-multiple-plus-outline' size={28} style={styles.newIcon}/></Button> }
          </React.Fragment>
    ...
    renderChefImage = (listChoice, chefID) => {
      ...
      if (chef != undefined){
        if (chef.imageURL != null) {
          ...
          return <Image style={[{width: '50%', height: 100}, styles.detailsImage]} source={require("../dataComponents/peas.jpg")}></Image>
        }
      }
    }
