...
describe("Recipe List", () => {
	...
	describe("rendering", () => {
		test("renders correctly when empty with an empty list and a filter button and adds appropriate listeners, then removes listeners when unmounted", async () => {
			await act(async () => {
				component = await mount(<RecipesList navigation={navigation} route={route} listChoice={"all"} />, {
		...
		test("renders correctly with the My Feed Message when it's my feed and empty", async () => {
			...
			await act(async () => {
				component = await mount(
					<RecipesList navigation={navigation} route={{ name: "My Feed" }} listChoice={"chef_feed"} />,
					{
						wrappingComponent: Provider,
						wrappingComponentProps: {
							store: store,
	...
	describe("liking and commenting etc.", () => {
		...
		test("liking a recipe calls to server and if successful adds a like count", async () => {
			...
			let otherChefLikes = instance.props.allRecipeLists[instance.props.route.key]
				.slice(1)
				.map((r) => r.chef_liked);
			...
			await act(async () => {
				await card.likeRecipe(recipe.id);
			});
			expect(recipe.likes_count).toEqual(4); // add one to the recipe
			expect(recipe.chef_liked).toEqual(1); //change to recipe
			expect(instance.props.allRecipeLists[instance.props.route.key].slice(1).map((r) => r.likes_count)).toEqual(
				otherLikes
			); //expect no changes to other likes
			expect(instance.props.allRecipeLists[instance.props.route.key].slice(1).map((r) => r.chef_liked)).toEqual(
				otherChefLikes
			); //expect no changes to other likes
		});
		...
		test("unliking a liked recipe calls to server and if successful subtracts a like count", async () => {
			...
			let otherLikes = [
				...instance.props.allRecipeLists[instance.props.route.key].slice(0, 1),
				...instance.props.allRecipeLists[instance.props.route.key].slice(2),
			].map((r) => r.likes_count);
			let otherChefLikes = [
				...instance.props.allRecipeLists[instance.props.route.key].slice(0, 1),
				...instance.props.allRecipeLists[instance.props.route.key].slice(2),
			...
			await act(async () => {
				await card.unlikeRecipe(recipe.id);
			});
			...
			).toEqual(otherLikes); //expect no changes to other likes
			expect(
				[
					...instance.props.allRecipeLists[instance.props.route.key].slice(0, 1),
					...instance.props.allRecipeLists[instance.props.route.key].slice(2),
				].map((r) => r.chef_liked)
			).toEqual(otherChefLikes); //expect no changes to other likes
		});
		...
		test("resharing a recipe calls to server and if successful adds a share count", async () => {
			...
			let otherShares = instance.props.allRecipeLists[instance.props.route.key]
				.slice(1)
				.map((r) => r.shares_count);
			let otherChefShares = instance.props.allRecipeLists[instance.props.route.key]
				.slice(1)
				.map((r) => r.chef_shared);
			...
			await act(async () => {
				await card.reShareRecipe(recipe.id);
			});
			expect(recipe.shares_count).toEqual(3); // add one to the recipe
			expect(recipe.chef_shared).toEqual(1); //change to recipe
			expect(instance.props.allRecipeLists[instance.props.route.key].slice(1).map((r) => r.shares_count)).toEqual(
				otherShares
			); //expect no changes to other likes
			expect(instance.props.allRecipeLists[instance.props.route.key].slice(1).map((r) => r.chef_shared)).toEqual(
				otherChefShares
			); //expect no changes to other likes
		});
		...
		test("unReSharing a liked recipe calls to server and if successful subtracts a share count", async () => {
			...
			let otherLikes = [
				...instance.props.allRecipeLists[instance.props.route.key].slice(0, 2),
				...instance.props.allRecipeLists[instance.props.route.key].slice(3),
			].map((r) => r.shares_count);
			let otherChefLikes = [
				...instance.props.allRecipeLists[instance.props.route.key].slice(0, 2),
				...instance.props.allRecipeLists[instance.props.route.key].slice(3),
			...
			await act(async () => {
				await card.unReShareRecipe(recipe.id);
			});
		...
		test("making a recipe calls to server and if successful adds a make count", async () => {
			...
			let otherChefMakes = instance.props.allRecipeLists[instance.props.route.key]
				.slice(1)
				.map((r) => r.chef_made);
			...
			await act(async () => {
				await card.makeRecipe(recipe.id);
			});
			expect(recipe.makes_count).toEqual(2); // add one to the recipe
			expect(recipe.chef_made).toEqual(1); //change to recipe
			expect(instance.props.allRecipeLists[instance.props.route.key].slice(1).map((r) => r.makes_count)).toEqual(
				otherMakes
			); //expect no changes to other likes
			expect(instance.props.allRecipeLists[instance.props.route.key].slice(1).map((r) => r.chef_made)).toEqual(
				otherChefMakes
			); //expect no changes to other likes
		});
